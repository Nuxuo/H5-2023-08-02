...
operating_time_start = setup_response_json["operatingAllowedTimeStampStart"]
operating_time_end = setup_response_json["operatingAllowedTimeStampEnd"]
...
conditions_last_send = conditions_last_check
...
def check_conditions(ts):
    ...
    if ( has_time_elasped( conditions_last_check , timestamp, conditions_mins_conditions )):
        ...
        payload = {'ServerRoomId':ServerRoomId,'Temperature':temp,'Humidtity':hum,'Datetime':time}
        response = requests.post(Api_Address + '/V1/Data/Conditions', json = payload)
        
    if (reason != ""):
        print("Sending Alarm..")
        lcd.lcd_puts(reason,4)
        time = dumps(timestamp, default=json_serial)
        payload = {'ServerRoomId':ServerRoomId,'Temperature':temp,'Humidtity':hum,'Datetime':time,'Reason':reason,}
        response = requests.post(Api_Address + '/V1/Data/Alarm', json = payload)
